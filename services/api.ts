...
const checkErrorNetwork = (err: any) => {
  if (err?.toJSON() && err.toJSON().message === 'Network Error') {
    return console.error(typeOfMessage.ERROR, getI18n().t(`message.E11`));
  }
  return err;
};
...
const checkErrorStatus = (response: any) => {
  ...
  if (response?.meta?.errorCode) {
    if (response?.meta?.errorCode !== 'E0') {
      console.error(typeOfMessage.ERROR, `message.${response?.meta?.errorCode}`, response?.meta?.extraInfo);
    } else {
      console.error(typeOfMessage.ERROR, response?.meta?.msg);
    }
  }
  return response;
};
